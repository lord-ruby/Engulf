...
Engulf.EditionFuncs = {
    ...
    e_cry_astral = Engulf.Editionfunc({mult=true,type="^"}, "talisman_emult", function(edition) return edition.e_mult end),
    e_cry_mosaic = Engulf.Editionfunc({chips=true,type="X"}, "cry_e_mosaic", function(edition) return edition.x_chips end),
    e_cry_oversat = function(card, hand, instant, amount, edition)
        G.GAME.hands[hand].chips = math.max(G.GAME.hands[hand].chips + (G.GAME.hands[hand].l_chips * (to_big(2)^to_big(amount))), 0)
        G.GAME.hands[hand].mult = math.max(G.GAME.hands[hand].mult + (G.GAME.hands[hand].l_mult * (to_big(2)^to_big(amount))), 1)
        if not instant then
    ...
    e_cry_glitched = function()
        ...
        * amount
        local modm = G.GAME.hands[hand].l_mult
            ...
            * amount
        G.GAME.hands[hand].chips = math.max(G.GAME.hands[hand].chips + modc*amount, 1)
        G.GAME.hands[hand].mult = math.max(G.GAME.hands[hand].mult + modm*amount, 1)
        if not instant then
...
function Engulf.EditionHand(card, hand, edition, amount, instant)
    if Engulf.EditionFuncs[edition.key] then Engulf.EditionFuncs[edition.key](card, hand, instant, Engulf.config.stackeffects and amount or 1, edition) end
end
